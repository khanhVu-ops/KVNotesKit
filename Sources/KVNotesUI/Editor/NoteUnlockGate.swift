    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: theme.large) {
                ...
                Text(denied ? "Not unlocked" : "Unlocking private note")
                ...
                Button(action: onUnlock) {
                    Label(isAuthenticating ? "Authenticating…" : "Unlock", systemImage: "lock.open")
                        .font(theme.modeFont).foregroundStyle(theme.onAccent).frame(maxWidth: .infinity).frame(height: 48)
                        .background(theme.accent, in: Capsule())
                }.disabled(isAuthenticating)
                Button("Cancel", action: onCancel).foregroundStyle(theme.secondaryText)
            }.padding(theme.large)
        }
    }
